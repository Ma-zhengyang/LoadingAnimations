...
public class StarIndicator extends IndicatorDrawable {
    ...
    protected void draw(Canvas canvas, Paint paint) {
        ...
        if (mAnimatedValue < 0.5) {//下落
            // 起始位置getHeight() / 4，终点 getHeight() / 2
            canvas.translate(getWidth() / 2, getHeight() / 4 + mAnimatedValue * (getHeight() / 2));
        } else {//弹起
            canvas.translate(getWidth() / 2, getHeight() / 4 + (1 - mAnimatedValue) * (getHeight() / 2));
        }
